...
def parse_csv():
    ...
    pref_df.columns = ['cand_id'] + [str(i) for i in range(1, K + 1)]

    slots_df.sort_values(by = 'start_time')

    time_id_map = {slots_df.iloc[i, 'start_time'] : i for i in range(N)}
    # Map timings in pref_df to their indices
    pref_df[1:] = pref_df[1:].applymap(lambda time: time_id_map[time])
